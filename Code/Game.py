...
sys.path.insert(0, "../Launcher.py")
from Launcher import path
...
# img_final.save('quit_screen.png')
cursorx = 0
cursory = 0
...
def draw_objects():
    quit_button.draw()
    arcade.draw_text(f"Score = {score}", 8, s_height - 76, arcade.color.RED_ORANGE, 38)
    arcade.draw_rectangle_outline((s_width // 2), (s_height // 2), ((s_width // 4) * 3), s_height,
                                  arcade.color.RED_ORANGE, 10)
    target.draw()
    cursor.draw()
...
class gam(arcade.Window):
    ...
    def on_mouse_press(self, x: float, y: float, button, modifiers: int):
        global quit_press, quit_button, is_bye, cursor
        quit = arcade.check_for_collision(quit_button, cursor)
        if quit:
    ...
    def on_update(self, delta_time: float):
        global target, targetY, targetX, is_bye
        ...
        if collide == True and self.clicked == True:
            ...
            score += 1
        quit = arcade.check_for_collision(quit_button, cursor)
        if quit == True and quit_press == True:
            bye.draw()
            bye.update()
